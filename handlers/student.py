from utilities.database_async import query_card_async, send_task_async, retrieve_tasks_async
from utilities.authorizing import is_registered, UserRole
from utilities.keyboard import createInlineTaskButton
from lexicon import lexicon
from filters import validate_file
...
async def sends_tasks(message: Message, state: FSMContext, db):
    ...
    if is_student:
        ...
        tasks_dict_of_dicts = await retrieve_tasks_async(db, level)
...
async def handle_document(message: Message, state: FSMContext, db):
    ...
    await send_task_async(db, username, task_id, file_bytes)
