...
def send_task(db: Client, username, task_id, file):
    current_time = datetime.now()
    unique_id = str(current_time.strftime("%Y%m%d_%H%M%S"))
    public_id = f"{username}{task_id}{unique_id}"
    data = {unique_id: public_id, "is_checked": False}
    ref = db.collection('tasklogs').document('IT')\
        .collection(str(username)).document(str(task_id)).set(data, merge=True)
    upload_file(file, username, task_id, public_id)

def retrieve_task(db: Client, level: str) -> dict:
    try:
        ref = db.collection('tasks').document('IT').collection(level)
        docs = ref.stream()
